...
(async() => {
    ...
    // IMAP의 로그인
    const imapLoginResult = await imapHandler.command("login").execute("gmlsdyd1023", "shwngh102@");
    ...
    // POP3의 로그인
    const pop3LoginResult = await pop3Handler.command("user").execute("gmlsdyd1023").then(user => {
        return pop3Handler.command("pass").execute("shwngh102@");
    });
    ...
    // IMAP 의 메일 박스 선택
    const r = await imapHandler.command("select").execute("inbox");
